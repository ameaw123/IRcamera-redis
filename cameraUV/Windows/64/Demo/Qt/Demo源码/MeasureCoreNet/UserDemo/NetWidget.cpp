...
int CALLBACK rgbCbFun(const GD_MTC_CALLBACK_RGBInfo * RGBInfo, void *pUser)
{
	...
	if (g_pTest3)
	{
		...
		g_bInRgbCb = true;
		emit g_pTest3->showImg(pData, RGBInfo->ImgWidth, RGBInfo->ImgHeight);
	}
...
int CALLBACK y16CbFun(const GD_MTC_CALLBACK_Y16Info * pY16Info, void *pUser)
{
	...
	{
		g_bInY16Cb = true;
		emit pWidget->y16Data(nAlarmMinFlag, nAlarmMaxFlag, fSpanMin, fSpanMax);
	}
...
int CALLBACK linkStateCbFun(const GD_MTC_CALLBACK_LinkStateInfo *LinkStateInfo, void *pUser)
{
	...
	if (-1 == n2)
	{
		// 未连接
		n1 = n1;
		emit pWidget->linkStateChange(0);
	}
	else if (0 == n2)
	{
		// 已连接
		n1 = n1;
		emit pWidget->linkStateChange(1);
	}
	n1 = n1;
	return 0;
}
...
int CALLBACK serarchDeviceCbFun(const GD_MTC_IRDeviceInfo * pDeviceInfo, int nDeviceInfoSize, void *pUser)
{
	...
	if (0 != pWidget)
	{
		if (nDeviceInfoSize > 0)
		{
			...
			emit pWidget->searchDeviceResult(pDeviceInfo2, nDeviceInfoSize, pUser);
		}
		else if (0 == nDeviceInfoSize)
		{
			emit pWidget->searchDeviceResult((GD_MTC_IRDeviceInfo *)0, 0, pUser);
		}
	}
...
void NetWidget::OnBnClickedEnableShutter()
{
	...
	if (GD_MTC_ERROR_CODE::NO_ERROR == nRtn)
	{
		int n1 = 0;
	}
	else
...
void NetWidget::onBtnClickedGetDeviceInfo()
{
	...
	char cBuf[200] = {};
	if (GD_MTC_ERROR_CODE::NO_ERROR == GD_MTC_SDK_GetSDKVersion(0, cBuf, 0))
	{
...
{
	GD_MTC_SDK_SendNetWorkCmd(m_nSessionId, Focus, 2, 0);
}

void NetWidget::onBtnPressedFarFocus()
{
	GD_MTC_SDK_SendNetWorkCmd(m_nSessionId, Focus, 1, 0);
}
...
{
	GD_MTC_SDK_SendNetWorkCmd(m_nSessionId, Focus, 0, 0);
}
